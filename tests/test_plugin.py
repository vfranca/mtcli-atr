...
def test_atr_saida_valida(monkeypatch):
    ...
    monkeypatch.setattr("mtcli_atr.plugin.mt5.copy_rates_from_pos", lambda *a, **kw: gerar_rates_fake(periodo))
    monkeypatch.setattr("mtcli_atr.plugin.conectar", lambda: None)
    monkeypatch.setattr("mtcli_atr.plugin.shutdown", lambda: None)

    runner = CliRunner()
    result = runner.invoke(atr, ["--symbol", symbol, "--periodo", str(periodo), "--timeframe", timeframe])
...
def test_dados_insuficientes(monkeypatch):
    periodo = 14
    monkeypatch.setattr("mtcli_atr.plugin.mt5.symbol_select", lambda s, v: True)
    monkeypatch.setattr("mtcli_atr.plugin.mt5.copy_rates_from_pos", lambda *a, **kw: [{"high": 110, "low": 100, "close": 105}] * 5)
    monkeypatch.setattr("mtcli_atr.plugin.conectar", lambda: None)
    monkeypatch.setattr("mtcli_atr.plugin.shutdown", lambda: None)
